...
class RepairRequestWindow(ctk.CTk):
    def __init__(self, on_close_callback):
        ...
        self.component_label = ctk.CTkLabel(self.form_frame, text="Комплектующие")
        self.component_label.grid(row=3, column=0, padx=10, pady=5, sticky='e')
        self.component_combobox = ctk.CTkComboBox(self.form_frame, values=["Материнская плата", "Видеокарта", "Жесткий диск", "Оперативная память", "Монитор", "Клавиатура", "Мышь", "Вебкамера", "Блок питания"])
        self.component_combobox.grid(row=3, column=1, padx=10, pady=5)
        ...
        self.problem_description_text.grid(row=4, column=1, padx=10, pady=5)
    ...
    def submit_request(self):
        ...
        components = self.component_combobox.get()
        problem_description = self.problem_description_text.get("1.0", 'end-1c')
